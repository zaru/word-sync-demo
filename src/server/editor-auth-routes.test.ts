...
import { createEditorAuthHandlers, type MicrosoftAuthBoundary } from './editor-auth-routes';
...
describe('Microsoft sign-in routes', () => {
  ...
  it('signs an 編集者 in after Microsoft callback without exposing raw token cache to browser code', async () => {
    const tokenCache = '{"RefreshToken":{"secret":"browser must not receive this"}}';
    ...
    const loginResponse = await handlers.login(new Request('http://localhost/auth/login'));
    ...
    const callbackResponse = await handlers.callback(
      new Request('http://localhost/auth/callback?code=auth-code&state=state-1', {
        headers: {
    ...
    expect(JSON.stringify(browserSession)).not.toContain('browser must not receive this');
  ...
  it('logs out the signed-in 編集者 and clears the browser session', async () => {
    ...
    expect(logoutResponse.headers.get('set-cookie')).toContain('word_sync_editor_session=');
    ...
    expect(store.readTokenCache('microsoft-account-1')).toBe('{"RefreshToken":{"cached":true}}');
  });
});
