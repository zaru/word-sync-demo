...
export function getEditorAuthStore(): EditorAuthStore {
  const databasePath =
    process.env.EDITOR_AUTH_DB_PATH ?? join(process.cwd(), '.data', 'editor-auth.sqlite');
...
export function readEditorSessionFromCookieValue(sessionId: string | undefined): EditorSession | undefined {
  if (!sessionId) {
    return undefined;
  }
