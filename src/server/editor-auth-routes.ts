	...
	return {
		async login(): Promise<Response> {
			...
			const authorizationUrl =
				await options.microsoftAuth.createAuthorizationUrl({
					redirectUri,
		...
		async callback(request: Request): Promise<Response> {
			...
			if (!code || !state || state !== expectedState) {
				return Response.json(
					{ error: "Invalid Microsoft sign-in callback" },
					{ status: 400 },
				);
			}

			const signedInEditor =
				await options.microsoftAuth.completeAuthorizationCode({
					code,
					redirectUri,
					scopes: editorAuthScopes,
				});
		...
		async session(request: Request): Promise<Response> {
			...
			const session = sessionId
				? options.store.readSession(sessionId)
				: undefined;
		...
		async logout(request: Request): Promise<Response> {
			...
			return redirectResponse(`${options.appBaseUrl}/`, [
				expireCookie(editorSessionCookieName),
			]);
		},
	};
}
