...
import { lexicalMarkdownNodes, markdownTransformers } from '../domain/lexical-markdown-bridge';
import { createDebouncedAutosave, loadChangedWebDocument } from './web-document-client';
...
export function WebDocumentEditor({ initialDocument }: WebDocumentEditorProps) {
  ...
  const autosave = useMemo(
    () =>
      createDebouncedAutosave({
        ...
        onError(error) {
          setStatus('error');
          setErrorMessage(error instanceof Error ? error.message : 'Autosave failed.');
        },
      }),
    [],
  );
  ...
  const initialConfig = useMemo(
    () => ({
      ...
      editorState() {
        $convertFromMarkdownString(initialDocument.markdown, markdownTransformers);
      },
    }),
...
function RichEditor() {
  return (
    <RichTextPlugin
      contentEditable={<ContentEditable aria-label="Markdown互換内容" className="editor-input" />}
      placeholder={<div className="editor-placeholder">Markdown互換内容を入力...</div>}
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      loadChangedWebDocument({
        ...
        editor.update(() => {
          $convertFromMarkdownString(changedDocument.markdown, markdownTransformers);
        });
...
async function fetchVersion(): Promise<Pick<WebDocument, 'version'>> {
  const response = await fetch('/api/web-document/version', { cache: 'no-store' });

  if (!response.ok) {
    throw new Error(`Could not read Webドキュメント version: ${response.status}`);
  }
...
async function readWebDocumentResponse(response: Response): Promise<WebDocument> {
  if (!response.ok) {
    throw new Error(`Could not save or load Webドキュメント: ${response.status}`);
  }
...
function isVersionResponse(body: unknown): body is Pick<WebDocument, 'version'> {
  return (
    typeof body === 'object' &&
    body !== null &&
...
function statusLabel(status: SaveStatus, errorMessage: string | undefined): string {
  if (status === 'saving') {
    return 'Autosaving...';
  }
