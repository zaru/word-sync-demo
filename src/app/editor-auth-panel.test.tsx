...
describe('EditorAuthPanel', () => {
  it('shows Microsoft login UI before an 編集者 signs in', () => {
    ...
    expect(screen.getByRole('link', { name: 'Microsoftでサインイン' })).toHaveAttribute(
      'href',
      '/auth/login',
    );
    expect(screen.getByText('Word起動導線はサインイン後に利用できます。')).toBeInTheDocument();
  });
  ...
  it('shows logout UI and keeps OneDrive-dependent actions behind the signed-in 編集者 session', () => {
    ...
    expect(
      screen.getByText('OneDrive作業コピーを使う操作は、この編集者セッションで実行できます。'),
    ).toBeInTheDocument();
