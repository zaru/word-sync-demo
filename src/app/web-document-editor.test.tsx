...
describe('WebDocumentEditor', () => {
  ...
  it('hydrates a non-empty Markdown互換内容 projection without a React mismatch', async () => {
    ...
    expect(consoleError.mock.calls.flat()).not.toContainEqual(
      expect.stringContaining("Hydration failed because the server rendered HTML didn't match the client"),
    );
  });
});

async function renderServerWebDocumentEditor(initialDocument: WebDocument): Promise<string> {
  vi.resetModules();
  vi.stubGlobal('window', undefined);
  vi.stubGlobal('document', undefined);

  try {
    const [{ createElement }, { renderToString }, { WebDocumentEditor }] = await Promise.all([
      import('react'),
      import('react-dom/server'),
      import('./web-document-editor'),
    ]);

    return renderToString(createElement(WebDocumentEditor, { initialDocument }));
  } finally {
    vi.unstubAllGlobals();
  }
}
...
async function hydrateClientWebDocumentEditor(
  ...
  const [{ act, createElement }, { hydrateRoot }, { WebDocumentEditor }] = await Promise.all([
    import('react'),
