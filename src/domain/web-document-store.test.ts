...
describe('WebDocumentStore', () => {
  ...
  function createStore(seedMarkdown = '# Shared Webドキュメント\n\n最初のMarkdown互換内容。') {
  ...
  it('persists autosaved Markdown and increments the version each time', () => {
    ...
    const firstSave = store.saveMarkdown('## Updated Webドキュメント\n\nAutosaved Markdown互換内容。');
    const secondSave = store.saveMarkdown('## Updated again\n\n別タブにも見える内容。');
